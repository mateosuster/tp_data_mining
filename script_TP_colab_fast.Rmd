...
barplot(table(df_charts_sel$popularidad_chart))
```
...
en_lyrics$bad_words_cat <- cut(en_lyrics$cant_bad_words, breaks = c(0,10,20,50,Inf), labels=c("Bajo","Medio","Alto","Muy alta"))
```
...
reglas_sub <- subset(reglas, subset = lhs %in% "danceability_cat" & rhs %pin% "popularidad")
...
#reglas encontradas
reglas_sub <- subset(reglas, subset = !(lhs %pin% "semanas_sum") & !(lhs %pin% "streams") & !(lhs %pin% "position")  
                     &   rhs %pin% "popularidad") # lhs =  {explicit_cat=No,genero=pop}  // lhs ={bad_words_cat=bajo,explicit_cat=No}

reglas_sub <- subset(reglas, subset = lhs %pin% "_cat" & rhs %pin% "popularidad")

reglas_sub <- subset(reglas, subset = lhs %pin% "_cat" & rhs %pin% "popularidad" & size(lhs) <2 ) 
...
reglas_sub <- subset(reglas, subset = lhs %pin% "bad_words_cat" & rhs %pin% "popularidad")# , size(lhs)==1 ) 
reglas_sub <- subset(reglas, subset = lhs %pin% "bad_words_cat") 
...
# summary(lyrics_trans)
reglas_esp <- apriori(lyrics_trans_esp, parameter = list(support=0.1,
                    confidence = 0.1, target  = "rules"  ))



reglas_sub_esp <- subset(reglas_esp, subset = lhs %pin% "_cat" & rhs %in% "position_median_chart=Alta")


reglas_sub_esp <- subset(reglas_esp, subset = lhs %pin% "TERM" &  rhs %pin% "genero") 
# lhs = TERM_noche
