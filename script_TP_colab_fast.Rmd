...
eng_word_counts_df <- read.csv("data/en_lyrics_word_count.csv")
...
```{r}
spa_word_counts_df <- read.csv("data/spa_lyrics_word_count.csv")
```
...
en_lyrics$bad_words_cat <- cut(en_lyrics$cant_bad_words, breaks = c(0,10,20,50,Inf), labels=c("Bajo","Medio","Alto","Muy alta"), include.lowest = T, right = F)
...
reglas_sub_esp <- subset(reglas_esp, subset = lhs %pin% "TERM" &  rhs %pin% "genero" & size(lhs)==1) 
# lhs = TERM_noche
