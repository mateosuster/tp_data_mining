...
```{r}
features_categoricos <-c( 'explicit', 'key_name', 'mode_name', "key_mode", "album_type")
...
```{r}
library(mice)
md.pattern(df_audio_features, rotate.names = T)
```


## experimento/ bosquejo de join
```{r}
features <- c('acousticness', 'danceability', 'duration_ms', 'energy', 'instrumentalness', 'liveness', 'loudness', 'speechiness',   'tempo', 'valence')

df_audio_features %>% 
  select("artist_name", "track_name", features) %>% 
  left_join( df_charts %>%
               select( "Track_Name", "Artist", "Position") %>%
