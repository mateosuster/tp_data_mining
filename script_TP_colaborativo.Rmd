...
features_continuas_mediana <- c('acousticness', 'duration_ms', 'energy', 'instrumentalness', 'liveness', 'loudness_reg_imp', 'speechiness', 'cant_markets')
...
df_datos <-df_datos[,order(colnames((df_datos)))]
...
sin_outliers_box <- function(col){
  ...
  bp <- boxplot(scale(col[conditions]), main=paste(name,"\nsin_out",sep='_'), horizontal = T)
  return(bp)
}
...
par(mfrow=c(2,3), mar=c(5,2,5,2))
for (feature in features_continuas_media){
  hist(df_audio_features[,feature], main=feature, xlab="")
}
for (feature in features_continuas_media){
  boxplot(df_audio_features[,feature], horizontal=T)
}
mtext("Distribuciones danceability, tempo, valence", side = 3, line = -1, outer = TRUE)
```
...
par(mfrow=c(4,4))
for (feature in features_continuas_mediana[1:4]){
  boxplot(scale(df_audio_features[,feature]), las=2, horizontal=T, main=feature)
}
for (feature in features_continuas_mediana[1:4]){
  sin_outliers_box(feature)
}
for (feature in features_continuas_mediana[5:8]){
  boxplot(scale(df_audio_features[,feature]), las=2, horizontal=T, main=feature)
}
for (feature in features_continuas_mediana[5:8]){
  sin_outliers_box(feature)
...
par(mfrow = c(2,1)) 
boxplot(df_audio_features[,'loudness'], main='loudness', horizontal = T)
boxplot(df_audio_features[,'loudness_reg_imp'], main='loudness_reg', horizontal = T)
```
...
df_audio_charts_cuanti <- join_audio_charts[,c("external_urls_spotify", "track_name", audio_charts_continuas_media, audio_charts_continuas_mediana)]
...
df_audio_charts_s_at_redund <- df_audio_charts_s_at_redund %>% 
  select(-c("duration_ms", "Streams", "loudness", "liveness", "speechiness", "cant_markets"))
...
ggbiplot::ggbiplot(df_audio_charts_s_at_redund.PCA, obs.scale=0.75 ,var.scale=0.75,alpha=0.05)
                   # main = "CPA de features del chart")
```
...
#que canciones identifico?
top_ifores_sample <- head(df_audio_charts_s_at_redund_scale[order(df_audio_charts_s_at_redund_scale$iforest_pred,
                                                                  decreasing = T),],200)


top_ifores_sample <- df_audio_charts_s_at_redund_scale[df_audio_charts_s_at_redund_scale$iforest_pred > 0.5,]

print(top_ifores_sample)

# freq_outlier <- as.data.frame(table(join_audio_charts[join_audio_charts$external_urls_spotify%in%
#                                                         top_ifores_sample$external_urls_spotify, c("external_urls_spotify")]))

outilers_pop <- popularidad[popularidad$URL%in% top_ifores_sample$external_urls_spotify, c("URL","indicador", "Track_Name" )]
top_outilers_pop <-  slice_max(.data = outilers_pop, n = 10, order_by = indicador)
...
plot(CP1, CP2,
     ...
     pch=ifelse(df_audio_charts_s_at_redund$external_urls_spotify %in% top_outilers_pop$URL,18,1),
     cex=ifelse(df_audio_charts_s_at_redund$external_urls_spotify %in% top_outilers_pop$URL,2,1),
     main = "Temas del chart sobre PCA y detección de outliers más populares")
legend( x = "bottomright", legend=c("Canciones del chart", "Outliers del chart", "Outliers del chart más populares"),
       col=c("blue", "red", "red"), pch = c(1, 1,18) , cex = 0.8)

# hist(freq_outlier$Freq)

hist((df_audio_charts_s_at_redund_scale$iforest_pred))
summary(top_ifores_sample$iforest_pred)
...
df_audio_charts_s_at_redund.PCA$rotation

```


```{r}
head(df_audio_charts_s_at_redund_scale[order(df_audio_charts_s_at_redund_scale$iforest_pred,
                                                                  decreasing = T),],200) %>% 
  arrange(-iforest_pred)

df_audio_features[df_audio_features$external_urls_spotify%in% top_outilers_pop$URL, ] %>% 
  select(track_name, instrumentalness, acousticness, danceability,valence,tempo )
```


```{r}
#GRAFICO VIEJO
#outlier con mayor frecuencia
plot(CP1, CP2,
     col=ifelse(df_audio_charts_s_at_redund$track_name %in% top_ifores_sample$track_name, "red", "blue"),
     pch=ifelse(df_audio_charts_s_at_redund$external_urls_spotify %in% freq_outlier_menos_de_10$Var1,18,1),
     cex=ifelse(df_audio_charts_s_at_redund$external_urls_spotify %in% freq_outlier_menos_de_10$Var1,2,1),
     main = "Temas del chart proyectados sobre las PCA y detección de outliers")
legend( x = "bottomright", legend=c("Canciones del chart", "Outliers del chart", "Outliers del chart con mayor frecuencia"),
       col=c("blue", "red", "red"), pch = c(1, 1,18) , cex = 0.8)
```
...
for (i in features_continuas){
 ...
 boxplot(art_af[,2], alb_af[,2], year_af[,2], names= c("artista", "album","year"), main =paste("Varianzas de",i), horizontal = TRUE, las=1)
}
...
for (i in (c("tempo", "acousticness", "danceability"))){
 ...
 boxplot(art_af[,2], art_CH[,2], alb_af[,2], alb_CH[,2], year_af[,2], year_CH[,2], names= c("artista_nonCH", "artista_CH", "album_nonCH", "album_CH", "year_nonCH", "year_CH"), main=paste("Varianzas de",i), horizontal = TRUE, las=1)
}
