...
variables_sesgo <- unlist(strsplit("danceability,tempo,valence,acousticness,duration_ms,energy,instrumentalness,liveness,speechiness,cant_markets", ","))
...
df_sesgadas_log_adjust <- as.data.frame(unlist(lapply(df_audio_features[,variables_sesgo], function(x) logaritmo_ajustado(x,delta))))
names(df_sesgadas_log_adjust) = paste(names(df_sesgadas), "log", sep="")

df_datos <- cbing(df_sesgadas, df_sesgadas_log_adjust)
