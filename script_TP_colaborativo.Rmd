...
ggbiplot::ggbiplot(df_audio_charts_s_at_redund.PCA, obs.scale=0.75 ,var.scale=0.75,alpha=0.05)+
  labs(title="Biplot de componentes principales")
                   # main = "CPA de features del chart")

summary(df_audio_charts_s_at_redund.PCA)
df_audio_charts_s_at_redund.PCA
```
...
for (i in (c("tempo", "acousticness", "danceability"))){
 ...
 boxplot(art_af[,2], art_CH[,2], alb_af[,2], alb_CH[,2], year_af[,2], year_CH[,2], names= c("artista_nonCH", "artista_CH", "album_nonCH", "album_CH", "year_nonCH", "year_CH"), main=paste("Varianzas de",i, "(distinción chart vs no chart)"), horizontal = TRUE, las=1)
}
...
x <- panel %>% 
  ...
  labs(title = "Dinámica del top 10 de canciones más populares", y = "Posición (en negativo)", x="")
...
y <- panel %>% 
  ...
  # geom_smooth()+
  theme(legend.position = "none", plot.title = element_text(size=10)) +
  labs(title = "Dinámica de las 150 canciones menos populares", x="", y="")
...
```{r}
df_charts %>% arrange(Artist) %>% 
  filter(Artist =="IU")

join_audio_charts %>% 
  filter(artist_name =="IU")
```
