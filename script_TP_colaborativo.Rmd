...
corrplot(x, type = "upper", title = "Correlacion de atributos de los Charts", mar=c(0,0,1,0), method="number" )
...
# umbral_zscore = 1
# df_audio_features[df_audio_features_zscore_media$zscore_speechiness> umbral_zscore,] %>% 
...
for (i in 1:ncol(df_audio_features_zscore_mediana)){
  ...
  df_audio_features_zscore_media[, i] <- 0.6745 * (df_audio_features_zscore_mediana[,i] - med) / MAD
  }


length(df_audio_features_zscore_media)
#################################
...
#audio features
df_audio_features_cuanti <- df_audio_features[,c(features_continuas_media, features_continuas_mediana)]

corr_audio_features <- cor(df_audio_features_cuanti, use = "complete.obs")
df_audio_features_s_at_redund <- findCorrelation(corr_audio_features, cutoff=0.60, names=TRUE, verbose= TRUE)

print(df_audio_features_s_at_redund) # Energy y Loudnes tienen alta correlación con otras variables.
                        # Las borramos!

df_audio_features_s_at_redund <- df_audio_features_cuanti %>% select(-c("energy", "loudness")) 

df_audio_features_s_at_redund <- cbind(df_audio_features$track_name, df_audio_features_s_at_redund)


#audio charts
df_audio_charts_cuanti <- join_audio_charts[,c(audio_charts_continuas_media, audio_charts_continuas_mediana)]

corr_charts <- cor(df_audio_charts_cuanti, use = "complete.obs")

df_audio_charts_s_at_redund <- findCorrelation(corr_charts, cutoff=0.60, names=TRUE, verbose= TRUE)

print(df_audio_charts_s_at_redund) # Energy y Loudnes tienen alta correlación con otras variables.
                                  # Las borramos!

df_audio_charts_s_at_redund <- df_audio_features_cuanti %>% select(-c("energy"))

df_audio_charts_s_at_redund <- df_audio_charts_s_at_redund[complete.cases(df_audio_charts_s_at_redund) == TRUE,]

x <- join_audio_charts[complete.cases(join_audio_charts) == TRUE,]
df_audio_charts_s_at_redund <- cbind(x$track_name, df_audio_charts_s_at_redund)
...
df_audio_features_s_at_redund.PCA = prcomp(df_audio_features_s_at_redund[,2:ncol(df_audio_features_s_at_redund)], scale. = TRUE)

round(df_audio_features_s_at_redund.PCA$sdev^2,2)

round(df_audio_features_s_at_redund.PCA$center,2)

round(df_audio_features_s_at_redund.PCA$scale,2)

df_audio_features_s_at_redund.PCA$rotation #autovectores de CPA

summary(df_audio_features_s_at_redund.PCA)

carga1 = data.frame(cbind(X=1:length(df_audio_features_s_at_redund[,2:ncol(df_audio_features_s_at_redund)]), primeracarga=data.frame(df_audio_features_s_at_redund.PCA$rotation)[,1]))

carga2 = data.frame(cbind(X=1:length(df_audio_features_s_at_redund[,2:ncol(df_audio_features_s_at_redund)]), primeracarga=data.frame(df_audio_features_s_at_redund.PCA$rotation)[,2]))

print(round(cbind(carga1,carga2),2))
