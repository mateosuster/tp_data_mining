...
x <- cor(df_audio_features[,features_continuas],  use =  "complete.obs")
corrplot(x, type = "upper", title = "Correlacion de atributos de audio", mar=c(0,0,1,0) )

x <- cor(join_audio_charts[,c(features_continuas, "Streams", "Position")], use =  "complete.obs")
...
df_audio_features_scale <- df_audio_features_filtrado[,features_continuas]

#normalizo z score
for(i in 1:ncol(df_audio_features_scale)){
  df_audio_features_scale[, i] <- (df_audio_features_scale[,i] -
                                   mean(df_audio_features_scale[,i]))/(sd(df_audio_features_scale[,i])) }

#analisis de z score
#variable: speechiness
umbral_zscore =3
df_audio_features[df_audio_features_scale$speechiness> umbral_zscore,] %>% 
...
for (i in features_continuas) {
  print(join_audio_charts_complete_scale %>%
            ...
            gather(key = variable, value = valor, 1:11) %>% 
            filter(variable== i ) %>%
              ggplot( aes(valor, color = is_chart))+
                labs(title = paste("histograma de", i),
                     subtitle = "Comparacion temas de chart vs no-chart")+
