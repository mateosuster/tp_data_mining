...
join_audio_artist <- df_audio_features %>% 
  select("artist_name", "track_name", features, "markets_concat") %>% 
...
##barplot de las variables categoricas
...
#cantidad de paises donde está disponible cada canción
...
summary(df_audio_features$cant_markets)
hist(df_audio_features$cant_markets)
boxplot(df_audio_features$cant_markets, horizontal = TRUE) #hay canciones en cero países.

df_audio_features[df_audio_features$cant_markets==0,]



#cantidad de paises donde está disponible cada canción del chart
...
join_audio_artist$cant_markets <- sapply(join_audio_artist[,"markets_concat"], contar_market)


summary(join_audio_artist$cant_markets)
hist(join_audio_artist$cant_markets)
boxplot(join_audio_artist$cant_markets, horizontal = TRUE) #hay canciones en cero países.

join_audio_artist[join_audio_artist$cant_markets==0,]
...
```{r}
summary(df_audio_features[, features_continuas])
...
features_continuas <- c('acousticness', 'danceability', 'duration_ms', 'energy', 'instrumentalness', 'liveness', 'loudness', 'speechiness',   'tempo', 'valence')
...
#ajusto el modelo
iforest_sample = isolation.forest(df_audio_features_filtrado, sample_size = 10000, ntrees=100, ndim = 3, random_seed = 13)
