...
## Sesgo DF charts
```{r}
df_ses_chart <- df_charts %>% select( "Track_Name", "Artist", 
...
cut(join_audio_charts$position_avg, quantile(join_audio_charts$position_avg, probs = seq(0, 1, 0.25
                                                          )),  labels = c("Muy Alta", "Alta", "Media", "Baja")) 
...
for(i in seq_along(df_num) ){
  ...
  breaks =unique(quantile(df_num[[i]], probs = seq(0, 1, 0.33)))
...
summary(df_num$popularidad)
quantile(df_num$popularidad)

summary(df_num$tempo)
quantile(df_num$tempo)

# seteo de nombres
names(df_cat) <- names(join_audio_charts)

join_audio_charts  %>% filter(track_name =="Juice WRLD Speaks From Heaven - Outro")
df_cat  %>% filter(track_name =="Baby")
df_cat  %>% filter(track_name =="goodbye")
df_cat  %>% filter(track_name =="DÁKITI")
join_audio_charts  %>% filter(track_name =="Rule The World (feat. Ariana Grande)")
...
df_ly_feat <- merge(x = cbind(en_lyrics[-c(3,4,5)], df_tm),
                    y =df_cat ,
...
### Español
```{r}
df_ly_feat_esp <- merge(x = cbind(spa_lyrics[-c(3,4,5)], df_tm_esp),
                    y =df_cat ,
                    by.x = c("artist_name","track_name"), 
                   by.y = c("artist_name","track_name"))
...
df_melt_esp <- reshape2::melt(data = df_ly_feat_ok_esp[,3:ncol(df_ly_feat_ok_esp)], id.vars = c("id"))  %>%
  arrange(id)
